...
class BattleSeniController extends BaseController
{
    ...
    private function viewData(array $data, string $title = 'Daftar Battle Seni'): array
    {
        return $data + ['title' => $title, 'activeMenu' => 'battle_seni', 'eventName' => (string) (get_setting('event_name') ?? 'Digital Pencak Silat'), 'eventLogo' => get_setting('event_logo', 'pendaftaran/gambar_dan_juknis'), 'adminName' => (string) (session()->get('nama') ?? session()->get('username') ?? 'Admin Sekretariat')];
    }
}
