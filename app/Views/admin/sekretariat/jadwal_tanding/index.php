    ...
    <div class="card-body px-0">
        <?php if (in_array(session()->get('level'), ['super_admin', 'sekretariat'], true)): ?>
            <div class="mb-3 d-flex flex-wrap gap-2">
                <?= view('shared_components/jadwal_tanding/modal_insert', ['data_gelanggang' => $gelanggang]) ?>
            </div>
        <?php endif; ?>
        ...
        <?php if (session()->get('level') === 'super_admin'): ?>
            <div class="mb-3">
                ...
                <button type="button" class="btn btn-danger mb-4" id="btnUpdateSelectedTanding" disabled>Update Selected PDF <span id="btnUpdateSelectedTandingCount">(0)</span></button>
                <button type="button" class="btn btn-success mb-4" id="btnUpdateSelectedTandingScore" disabled>Update Selected PDF with Score <span id="btnUpdateSelectedTandingScoreCount">(0)</span></button>
            </div>
        <?php endif; ?>

        <?= view('shared_components/jadwal_tanding/tabel', ['data_jadwal_tanding' => $rows]) ?>
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function processPDFSequential(jadwalList, withScore = false) {
        ...
        for (let i = 0; i < jadwalList.length; i++) {
            ...
            try {
                ...
                const url = '<?= base_url("admin/sekretariat/jadwal-tanding/create-pdf-ajax/") ?>' + jadwal.id + '/' + (withScore ? '1' : '0');
    ...
    async function processAllPDF(withScore = false) {
        ...
        try {
            const response = await fetch('<?= base_url("admin/sekretariat/jadwal-tanding/get-all-ids-ajax") ?>');
