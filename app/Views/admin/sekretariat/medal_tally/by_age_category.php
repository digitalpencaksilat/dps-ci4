...
<section class="admin-card mb-4">
    <div class="d-flex flex-column flex-lg-row justify-content-between gap-3 mb-4">
        <div>
            ...
            <h2 class="section-title h3 mb-2"><?= esc($reportTitle ?? $title ?? 'Perolehan Medali Per Kategori Usia') ?></h2>
        ...
        <div class="tab-content">
            <?php foreach ($categoryRows as $categoryName => $rows) : ?>
                ...
                <div class="tab-pane fade <?= array_key_first($categoryRows) === $categoryName ? 'show active' : '' ?>" id="<?= esc($tabId) ?>" role="tabpanel">
                    ...
                    <?= view('shared_components/medal_tally/contingent_table', ['rows' => $rows, 'tableId' => $tabId . '-table']) ?>
                </div>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
