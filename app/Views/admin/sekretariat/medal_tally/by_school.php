...
$reportName = (string) ($reportTitle ?? $title ?? 'Perolehan Medali Berdasarkan Sekolah');
    ...
    <?php else : ?>
        ...
        <div class="tab-content">
            <?php foreach ($categoryRows as $categoryName => $rows) : ?>
                <?php
                $tabId = 'school-category-' . md5($categoryName);
                $tableId = $tabId . '-table';
                $exportTables[] = [
                    'id' => $tableId,
                ...
                <div class="tab-pane fade <?= array_key_first($categoryRows) === $categoryName ? 'show active' : '' ?>" id="<?= esc($tabId) ?>" role="tabpanel">
                    ...
                    <?= view('shared_components/medal_tally/school_table', ['rows' => $rows, 'tableId' => $tableId]) ?>
                </div>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
