    ...
    <div class="tab-content">
        <?php if ($isBattle) : ?>
            <div class="tab-pane fade show active" id="battleSeni">
                ...
                <div class="table-responsive"><table class="table admin-table admin-datatable align-middle mb-0"><thead><tr><th>Babak</th><th>No</th><th>Merah</th><th>Biru</th><th>Partai</th><th>Gelanggang</th><th class="text-end">Aksi</th></tr></thead><tbody><?php foreach (($battleRows ?? []) as $battle) : ?><tr><td><?= esc($battle->babak ?? '-') ?></td><td><?= esc((string) ($battle->nomor_battle ?? '-')) ?></td><td><?= esc(($battle->anggota_kelompok_peserta_seni_merah ?? '-') . ' / ' . ($battle->nama_kontingen_merah ?? '-')) ?></td><td><?= esc(($battle->anggota_kelompok_peserta_seni_biru ?? '-') . ' / ' . ($battle->nama_kontingen_biru ?? '-')) ?></td><td><?= esc((string) ($battle->nomor_partai ?? '-')) ?></td><td><?= esc($battle->nama_gelanggang ?? '-') ?></td><td class="text-end"><a class="btn btn-sm btn-outline-danger rounded-pill" href="<?= base_url('admin/sekretariat/battle-seni/' . $battle->id_battle_seni) ?>">Detail</a></td></tr><?php endforeach; ?></tbody></table></div>
            </div>
        <?php else : ?>
