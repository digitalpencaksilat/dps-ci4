...
<div class="kartu-id-export"
     data-filename="<?= esc($card_filename ?? ('Kartu_Tanding_' . (int) ($peserta->id_peserta_tanding ?? 0))) ?>">
...
(function () {
    ...
    var FILENAME = <?= json_encode($card_filename ?? ('Kartu_Tanding_' . (int) ($peserta->id_peserta_tanding ?? 0)), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
    var BARCODE_VALUE = <?= json_encode($barcode_value ?? '', JSON_UNESCAPED_SLASHES) ?>;
    var BARCODE_SELECTOR = '#bar_tanding_<?= (int) ($peserta->id_peserta_tanding ?? 0) ?>';
    ...
    window.addEventListener('load', function () {
        ...
        // Auto-download supaya parity dengan flow legacy yang langsung print
