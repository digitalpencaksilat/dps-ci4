<div class="dropdown">
    ...
    <ul class="dropdown-menu dropdown-menu-end border-0 shadow-sm rounded-4">
        <li>
            <a class="dropdown-item" href="<?= base_url('admin/sekretariat/jadwal-seni/' . $jadwal->id_jadwal_seni) ?>">
        ...
        <?php if (session()->get('level') === 'super_admin'): ?>
            ...
            <li>
                <form action="<?= base_url('admin/sekretariat/jadwal-seni/' . $jadwal->id_jadwal_seni . '/delete') ?>" method="post" onsubmit="return confirm('Apakah Anda yakin? Schedule will be deleted!')">
                    <?= csrf_field() ?>
