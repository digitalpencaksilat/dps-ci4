...
| Admin super pembuatan jadwal | Present | Partial | Partial | Present | Partial | Missing/Manual | Partial | Core mode/routes/dashboard/drawing/jadwal wrapper sudah aktif di CI4. Audit terbaru: flow jadwal tanding super admin sudah reuse view sekretariat via `PembuatanJadwalController`, dengan create/update/delete, PDF export, sortir ulang nomor partai, pola penjadwalan, dan tukar atlet. Dokumentasi flow baru: `docs/flow-penjadwalan-otomatis-tanding-ci4.md`. Update implementasi: entrypoint parity CI3 untuk penjadwalan otomatis tanding sudah ditambahkan di CI4 via GET `admin/super/jadwal-tanding/penjadwalan-otomatis`, POST `admin/super/jadwal-tanding/buat-jadwal-tanding-otomatis`, view `app/Views/admin/super/jadwal_tanding/penjadwalan_tanding_otomatis.php`, controller `PenjadwalanTandingOtomatisController`, service `JadwalTandingOtomatisService`, migration baru `CreatePerangkatPenilaianTandingTables`, model `PerangkatPertandinganModel` + `PenilaianTandingModel`, dan service `PenilaianTandingService` untuk parity penugasan juri. Update QA: runtime verified di DB `db_testing_event`.
- Fix penting: `PenilaianTandingModel` dan `PerangkatPertandinganModel` harus `useTimestamps=false` (tabel tidak punya created_at/updated_at).
- Skema aktif: metadata jumlah juri diambil via join sampai `kategori_lomba.jumlah_juri` (bukan kolom di `pertandingan`).
Update QA: runtime verified juga untuk mode `pemasalan` + multi gelanggang (2 gelanggang, 2 partai masing-masing) di DB `db_testing_event`.\nGap tersisa: QA untuk opsi PDF (`langsung_buat_pdf`) via controller (web) belum jalan. |
