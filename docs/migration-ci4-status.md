...
| Admin super pengaturan kategori lomba | Present | Partial | Partial | Present | Needs Review | Syntax OK/Manual Missing | Partial | Stage 4 masih partial tetapi lebih lengkap: mode pengaturan kategori lomba memakai session historis `perngaturan_kategori_lomba`; kategori usia kini punya route/controller/view detail (`show`) selain create/edit/delete; kategori lomba dan sub kategori seni tetap tersedia di `admin/super`, dan create sub kategori seni tetap membuat pool awal `kompetisi_seni` dalam transaction. Manual QA, automated tests, dan CI3 parity review masih pending. |
