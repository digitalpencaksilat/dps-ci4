const juri = {
  ...
  update_offline_ui: function(is_offline) {
    ...
    if ($indicator.length > 0) {
      if (is_offline) {
        $indicator.removeClass('bg-success').addClass('bg-danger blink-indicator').html('<small><i class="fas fa-exclamation-triangle me-1"></i> Lokal</small>').show();
      } else {
        $indicator.removeClass('bg-danger').addClass('bg-success blink-indicator').html('<small><i class="fas fa-wifi me-1"></i> Online</small>').show();
      }
    }
  },
  ...
  update_data_nilai: function ($element) {
    /**
     * FIXED: Jury submissions now exclude penalty data to prevent race conditions.
     * Only technical scoring elements are sent to server. Penalties are controlled by KP only.
     */
    juri.hitung_total_nilai();
    
    // AKTIFKAN KEMBALI SECARA LOKAL: Optimistic UI.
    // Tujuannya murni visual: Juri tidak akan panik karena Nilai Akhir tidak stuck/membeku saat internet lambat.
    // Jangan khawatir: Angka lokal ini TIDAK masuk ke perhitungan 'clean_data' yang disubmit ke server.
    if (juri.data_nilai.penilaian.ringkasan.total_hukuman === undefined) {
      juri.hitung_total_hukuman();
    }
    juri.hitung_nilai_akhir();

    // Create clean data object without penalty information
    var clean_data = {
      penilaian: {
        ...
        ringkasan: {
          ...
          // Excluded: total_hukuman, nilai_akhir - backend will handle these
        },
        // Excluded: hukuman - only KP can modify penalties
      },
    };
    ...
    $.post(
      "juri/edit-penilaian-seni/" + juri.id_penampilan_seni,
      {
        ...
        // Removed nilai_akhir from submission - backend calculates this
      },
      function (data, textStatus, jqXHR) {
        // $($element).removeAttr('disabled');
        if (data && data.status == true) {
          // Success - penalty values will be updated via refresh_status_seni
          console.log("Technical scoring updated successfully");
          if (juri.is_offline) juri.set_online_status(true); // Switch back to online if came from offline
        } else {
    ...
    // Update display but preserve penalty information from last server sync
    juri.update_tampilan_nilai();
    $data_pointer = juri.pointer.get_data_pointer();
    juri.pointer.update_tampilan_pointer($data_pointer);
  },
  ...
  edit_hukuman: function ($jenis_hukuman, $data, $element) {
    /**
     * FIXED: Juries can no longer edit penalties. Only KP (Ketua Pertandingan) has authority.
     * This function now shows a notification instead of modifying penalty values.
     * This prevents race condition bugs in penalty synchronization.
     */

    // Show notification that penalty editing is not allowed for juries
    if (typeof Swal !== "undefined") {
      Swal.fire({
        title: "Akses Ditolak",
        text: "Hanya Ketua Pertandingan yang dapat mengubah nilai hukuman. Nilai hukuman akan disinkronkan secara otomatis.",
        icon: "info",
    ...
    console.log(
      "Jury attempted to edit penalty:",
      $jenis_hukuman,
      "Data:",
      $data,
    );

    return false; // Prevent any penalty modifications
  },
        ...
        $.each(
          ...
          function (key_jenis_unsur_seni, value_jenis_unsur_seni) {
            if (key_jenis_unsur_seni == "kebenaran") {
              ...
              // Handle tombol pilihan nilai (highlighting)
              $(".btn_pilihan_nilai_" + key_jenis_unsur_seni)
                .removeClass("btn-success shadow-sm fw-bold text-white")
                .removeClass(
                  "bg-gradient-180-blue bg-gradient-180-red bg-gradient-180-warning",
                )
                .addClass("btn-outline-dark");

              $(
                ".btn_pilihan_nilai_" +
                  key_jenis_unsur_seni +
                  "[data-nilai='" +
                  value_jenis_unsur_seni.nilai_diperoleh.toFixed(2) +
                  "']",
              )
                .removeClass("btn-outline-dark")
                .addClass(juri.kelas_aksen_warna + " text-white shadow-sm fw-bold");
            }
          },
        );
  ...
  update_akses_penilaian: function ($akses_penilaian) {
    if ($akses_penilaian == "dibuka") {
      ...
      if (!overlay) {
        ...
        overlay.id = "overlay";
        overlay.className =
          "position-fixed top-0 start-0 w-100 h-100 bg-dark d-flex justify-content-center align-items-center animated slideInDown";
  ...
  refresh_status_seni: () => {
    // Cek dan eksekusi sync data offline (jika ada) sebelum fetch status terbaru
    ...
    $.post(
      ...
      function (data) {
        ...
        /**
         * Authoritative sync:
         * - Keep ONLY local technical scoring (unsur_nilai) for the UI
         * - Replace hukuman + ringkasan (total_hukuman, nilai_akhir) from server
         * - Do NOT compute nilai_akhir on the client
         */
        try {
          // Preserve current technical scoring (user edits)
          const technicalScoring = juri.data_nilai.penilaian.unsur_nilai;

          // Take server-authoritative penalties and final
          const serverPenalties = data.data_nilai.penilaian.hukuman;
          const serverRingkasan = data.data_nilai.penilaian.ringkasan;

          // Overwrite with server truth
          juri.data_nilai.penilaian.hukuman = serverPenalties;
          juri.data_nilai.penilaian.ringkasan.total_hukuman =
            serverRingkasan.total_hukuman;
          juri.data_nilai.penilaian.ringkasan.nilai_akhir =
            serverRingkasan.nilai_akhir;

          // Restore local technical, then recalc ONLY total_nilai for display
          juri.data_nilai.penilaian.unsur_nilai = technicalScoring;
          juri.hitung_total_nilai(); // no hukuman / no final calc on client

          // Update UI using server-provided nilai_akhir
          juri.update_tampilan_nilai_akhir();
          juri.update_akses_penilaian(data.penampilan_seni.akses_penilaian);

          console.log(
            "Penalty data synchronized from server. Total penalties:",
            juri.data_nilai.penilaian.ringkasan.total_hukuman,
          );
        } catch (e) {
          console.error("Sync error:", e);
          // Fallback to hard reload if something is corrupt
          window.location.reload();
        }
      },
      "json",
      ...
      .fail(function () {
        console.error("Failed to refresh jury status - network or server error");
        // Otomatis ubah UI ke mode Offline saat server tidak bisa di-ping!
  ...
  /**
   * Toggle status ready juri.
   * Dipanggil saat juri menekan tombol READY/STANDBY di halaman penilaian seni.
   * Status ini dikirim ke server dan dapat dipantau oleh Ketua Pertandingan secara real-time.
   *
   * @param {HTMLElement} $btn - Element tombol yang diklik
   */
  toggle_ready: function($btn) {
    const currentStatus = parseInt($($btn).attr('data-status') || '0');
    const newStatus = currentStatus === 1 ? 0 : 1;

    // Disable tombol sementara untuk hindari double-click
    $($btn).prop('disabled', true);

    $.post(
      'juri/toggle-ready-seni/' + juri.id_penampilan_seni,
      { status_ready: newStatus },
      function(data) {
        if (data && data.status === true) {
          ...
          console.warn('Gagal toggle ready, status server:', data);
        }
      },
      'json'
    ).fail(function() {
