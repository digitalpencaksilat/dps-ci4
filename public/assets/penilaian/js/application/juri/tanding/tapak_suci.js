...
const juri = {
	 ...
	 * !!! script ini hanya untuk format penilaian Tapak Suci
	 */
	data_nilai: null,
	data_waktu: null,
	waktu_sekarang: null,
	...
	update_tampilan_nilai: function () {
		...
		$('.btn-nilai-merah, .btn-nilai-biru').removeAttr('disabled');
		$('.btn-nilai-merah').addClass('bg-yellow');
		$('.btn-nilai-biru').addClass('bg-blue text-white');
		...
		$.each(juri.data_nilai, function (sudut, v) {
			$.each(v.ronde_pertandingan, function (ronde, nilai) {
				...
				// if(v.ronde_pertandingan[juri.ronde_pertandingan].ringkasan.nilai_akhir > 200){
				// 	$('.btn-nilai-merah, .btn-nilai-biru').attr('disabled', true);
				// 	$('.btn-nilai-merah').removeClass('bg-yellow');
				// 	$('.btn-nilai-biru').removeClass('bg-blue text-white');
				// }
			});
		...
		juri.update_nilai_babak();

	},
	set_ronde: function(){
		$('td.ronde-1, td.ronde-2, td.ronde-3').removeClass('bg-red text-white');
		$('td.ronde-'+juri.ronde_pertandingan).addClass('bg-red text-white');
	},
			...
			$.each(v.ronde_pertandingan, function (ronde, value_ronde) {
				...
				$.each(value_ronde.rincian, function (index_nilai, nilai) {
					if (parseInt(nilai['nilai']) < 0) {
						...
						if (nilai['nilai'] == '10+20') {
							$total_nilai_ronde += 30;	
						} else if (nilai['nilai'] == '10+30') {
							$total_nilai_ronde += 40;
						} else if (nilai['nilai'] == '10+40') {
							$total_nilai_ronde += 50;
						} else {
	...
	pilih_pemenang: function ($temp_nilai){
		/**
		 * !!! SUDUT MERAH JANGAN DIGANTI KUNING
	...
	update_nilai_babak: function($pemenang){
		$('#skor_kuning').html(juri.pertandingan.skor_merah);
		$('#skor_biru').html(juri.pertandingan.skor_biru);
	},
