const ketua_pertandingan = {
	...
	init: function ($id_penampilan_seni, $data_nilai, $penampilan_seni_berlangsung, $autorefresh = true) {
		/**
	...
	update_tampilan_nilai: function ($data_nilai) {
		...
		$waktu_tampil = parseInt(ketua_pertandingan.penampilan_seni_berlangsung.waktu_tampil);
		$menit = parseInt($waktu_tampil / 60);
		$detik = parseInt($waktu_tampil % 60);
		$waktu_tampil = $menit.toString().padStart(2, '0') + ':' + $detik.toString().padStart(2, '0');

		$('.waktu_tampil_' + ketua_pertandingan.penampilan_seni_berlangsung.id_penampilan_seni).html($waktu_tampil);
		$('.nilai_akhir_' + ketua_pertandingan.penampilan_seni_berlangsung.id_penampilan_seni).html(ketua_pertandingan.penampilan_seni_berlangsung.nilai_akhir);

		if (ketua_pertandingan.penampilan_seni_berlangsung.diskualifikasi == 1) {
			$('.keterangan_' + ketua_pertandingan.penampilan_seni_berlangsung.id_penampilan_seni).html('<span class="badge badge-danger">Diskualifikasi</span>');
		} else {
			$('.keterangan_' + ketua_pertandingan.penampilan_seni_berlangsung.id_penampilan_seni).html(' ');
		}
		...
		$.each($data_nilai, function (id_penampilan_seni, penampilan_seni) {
			...
			$.each(penampilan_seni, function (index_juri, penilaian_juri) {
				...
				$('.penampilan_seni_' + id_penampilan_seni + ' .nilai_akhir_juri_' + penilaian_juri.id_perangkat_pertandingan).html($penilaian.ringkasan.nilai_akhir);
			});
		});
	...
	pilih_penilaian_juri: function ($data_nilai) {
		 ...
		 */
		$('tbody tr').removeClass('bg-gradient-180-red  text-white text-decoration-line-through');
		...
		$.each($data_nilai, function (id_penampilan_seni, penampilan_seni) {
			...
			$.each(penampilan_seni, function (index_juri, penilaian_juri) {
				if (penampilan_seni.length == 3) {
					...
					$('tbody tr.penampilan_seni_' + ketua_pertandingan.penampilan_seni_berlangsung, id_penampilan_seni).addClass('bg-gradient-180-red  text-white');
				} else {
					$.each(penampilan_seni, function (index_juri, penilaian_juri) {
						if (penilaian_juri.terpilih == 1 && id_penampilan_seni == ketua_pertandingan.penampilan_seni_berlangsung.id_penampilan_seni) {
							$('tbody tr.penampilan_seni_' + id_penampilan_seni + ' .juri_' + penilaian_juri.id_perangkat_pertandingan).addClass('bg-gradient-180-red  text-white');
						} else if (penilaian_juri.terpilih == 0 && id_penampilan_seni !== ketua_pertandingan.penampilan_seni_berlangsung.id_penampilan_seni) {
							//ketika bukan juri terpilih, dan penampilan sedang TIDAK BERLANGSUNG
							$('tbody tr.penampilan_seni_' + id_penampilan_seni + ' .juri_' + penilaian_juri.id_perangkat_pertandingan).addClass('text-decoration-line-through');
						} else {
	...
	update_unsur_nilai: function ($data_nilai) {
		...
		$.each($data_nilai, function (id_penampilan_seni, penampilan_seni) {
			...
			$.each(penampilan_seni, function (index_juri, penilaian_juri) {
				...
				$.each($penilaian.unsur_nilai, function (jenis_unsur_nilai, unsur_nilai) {
					$('.penampilan_seni_' + id_penampilan_seni + ' .' + jenis_unsur_nilai + '_juri_' + penilaian_juri.id_perangkat_pertandingan).html(unsur_nilai.nilai_diperoleh);
			...
			$.each(penampilan_seni, function (index_juri, penilaian_juri) {
				...
				$.each($penilaian.unsur_nilai, function (jenis_unsur_nilai, unsur_nilai) {
					if (penilaian_juri.terpilih == 1) {
						$total_nilai[id_penampilan_seni][jenis_unsur_nilai] += unsur_nilai.nilai_diperoleh;
					}
				});
			});
		});
		...
		$.each($total_nilai, function (id_penampilan_seni, unsur_nilai) {
			$.each(unsur_nilai, function (jenis_unsur_nilai, nilai_diperoleh) {
				$('.penampilan_seni_' + id_penampilan_seni + ' .total_nilai_' + jenis_unsur_nilai).html(nilai_diperoleh);
			});
		});
	},
	update_jenis_hukuman: function ($data_nilai) {
		...
		$.each($total_hukuman, function (id_penampilan_seni, data_hukuman) { 
			...
			$('.penampilan_seni_' + id_penampilan_seni + ' .total_hukuman').html(data_hukuman['total_hukuman']);
		});
	},
	refresh_status_seni: () => {
		$.post("perangkat-pertandingan/refresh-status-seni/" + ketua_pertandingan.id_penampilan_seni_berlangsung,
			function (data, textStatus, jqXHR) {
				if (data.status === true && data.reload === true) {
					location.reload();
				} else if (data.status === false && typeof data.data_nilai !== 'undefined') {
					ketua_pertandingan.id_penampilan_seni_berlangsung = data.penampilan_seni_berlangsung.id_penampilan_seni;
					ketua_pertandingan.penampilan_seni_berlangsung = data.penampilan_seni_berlangsung;
