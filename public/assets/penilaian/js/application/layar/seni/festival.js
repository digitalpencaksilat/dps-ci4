const layar = {
  ...
  update_tampilan_nilai: function () {
    ...
    $.each(
      ...
      function (index_juri, penilaian_juri) {
        $penilaian = JSON.parse(penilaian_juri.penilaian).penilaian;
        $.each(
          $penilaian.unsur_nilai,
          function (jenis_unsur_nilai, value_unsur_nilai) {
            $(
              "." +
                jenis_unsur_nilai +
                "_" +
                penilaian_juri.id_perangkat_pertandingan
            ).html(value_unsur_nilai.nilai_diperoleh);
          }
        );

        $(".nilai_akhir" + "_" + penilaian_juri.id_perangkat_pertandingan).html(
          $penilaian.ringkasan.nilai_akhir
        );
        $(
          ".total_hukuman" + "_" + penilaian_juri.id_perangkat_pertandingan
        ...
        if (
          ...
          if (penilaian_juri.terpilih == 1) {
            $(".juri_" + penilaian_juri.id_perangkat_pertandingan)
              .addClass("bg-warning text-white fw-bolder")
              .removeClass("text-decoration-line-through");
          } else {
            $(".juri_" + penilaian_juri.id_perangkat_pertandingan)
              .addClass("text-decoration-line-through")
              .removeClass("bg-warning text-white fw-bolder");
          }
        }
      }
    );
