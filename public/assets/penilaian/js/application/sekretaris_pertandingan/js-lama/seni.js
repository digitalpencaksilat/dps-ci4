const sekretaris_pertandingan = {
	...
	init: function ($penampilan_seni_berlangsung, $waktu_tampil) {
		...
		if($('#modal_penentuan_juara').length > 0 && $('#modal_ganti_format_penilaian').length > 0){
			sekretaris_pertandingan.modal_penentuan_juara = new bootstrap.Modal(document.getElementById('modal_penentuan_juara'));
			sekretaris_pertandingan.modal_ganti_format_penilaian = new bootstrap.Modal(document.getElementById('modal_ganti_format_penilaian'));
		}
		
		sekretaris_pertandingan.set_variable($penampilan_seni_berlangsung, $waktu_tampil)
	...
	set_variable: function ($penampilan_seni_berlangsung, $waktu_tampil) {
		sekretaris_pertandingan.id_penampilan_seni = $penampilan_seni_berlangsung.id_penampilan_seni;
		sekretaris_pertandingan.sistem_penampilan = $penampilan_seni_berlangsung.sistem_penampilan;
		sekretaris_pertandingan.status_penampilan = $penampilan_seni_berlangsung.status_penampilan;
	...
	toggle_timer: function () {
		$status_penampilan = $('.btn-toggle-waktu-tampil').data('status-penampilan'); // status penampilan yang akan dituju
		$waktu_sekarang = sekretaris_pertandingan.stopwatch.data('seconds');

		$.post("sekretaris-pertandingan/toggle-timer-seni/" + sekretaris_pertandingan.id_penampilan_seni,
			...
			function (data, textStatus, jqXHR) {
				if (data.status == true) {
					if ($status_penampilan == 'sedang_tampil') {
						...
						sekretaris_pertandingan.stopwatch.timer('start');
						$('.btn-toggle-waktu-tampil').data('status-penampilan', 'berhenti')
						$('.btn-toggle-waktu-tampil').html('pause').addClass('btn-danger').removeClass('btn-success');
					}
						...
						$('.btn-toggle-waktu-tampil').html('resume').addClass('btn-success').removeClass('btn-danger');
				...
				$.post("sekretaris-pertandingan/toggle-timer-seni/" + sekretaris_pertandingan.id_penampilan_seni,
					...
					function (data, textStatus, jqXHR) {
						if (data.status == true) {
							...
							Swal.fire('Error', 'Failed to reset clock, System error !', 'error');
						}
					}, "json"
				);
			}
	...
	mulai_penampilan_seni: ($id_penampilan_seni) => {
		Swal.fire({
			...
			if (result.value) {
				$.post("sekretaris-pertandingan/mulai-penampilan/" + $id_penampilan_seni,
	...
	selesai_penampilan: function () {
		Swal.fire({
			...
			if (result.value) {
				$.post("sekretaris-pertandingan/selesaikan-penampilan-seni/" + sekretaris_pertandingan.id_penampilan_seni,
					function (data, textStatus, jqXHR) {
						if (data.status == true && data.input_medali == false) {
							setTimeout(() => {
								Swal.fire({
									...
									$('.btn_selesai, .button-timer, .btn-diskualifikasi').prop('disabled', true);
								});
							}, 1000);
							...
							$('.btn_selesai, .button-timer, .btn-diskualifikasi').prop('disabled', true);
							setTimeout(() => {
								sekretaris_pertandingan.open_modal_input_juara(sekretaris_pertandingan.id_penampilan_seni);
							}, 4000);
						} else {
							Swal.fire('Error', data.message, 'error');
						}
	...
	pindah_partai: function ($partai_selanjutnya) {
		$.post("sekretaris-pertandingan/pindah-partai-seni", {
	...
	open_modal_input_juara: function () {
		...
		$.getJSON("sekretaris-pertandingan/get-data-penentuan-juara/"+sekretaris_pertandingan.sistem_penampilan+"/" + sekretaris_pertandingan.id_penampilan_seni,
			function (data, textStatus, jqXHR) {
				if(sekretaris_pertandingan.sistem_penampilan == 'pool'){
					...
					$('#penampilan_seni_biru, #penampilan_seni_merah').removeAttr('checked');

					if(data.penampilan_seni_biru.nilai_akhir > data.penampilan_seni_merah.nilai_akhir){
						$('#penampilan_seni_biru').prop('checked', true);
					}else if (data.penampilan_seni_biru.nilai_akhir < data.penampilan_seni_merah.nilai_akhir){
	...
	submit_input_juara_seni: function () {
		if(sekretaris_pertandingan.sistem_penampilan == 'pool'){	
			...
			$.ajax({
				...
				success:function(data, textStatus, jqXHR){
					...
					if (data.status == true) {
						...
						Swal.fire('success', 'Medals data inputted successfully', 'success');
					}
				},
			...
			$.post("sekretaris-pertandingan/pilih-pemenang-battle-seni/"+sekretaris_pertandingan.id_penampilan_seni,
	...
	diskualifikasi_peserta: function () {
		Swal.fire({
			...
			if (result.value) {
				$.post("sekretaris-pertandingan/diskualifikasi-penampilan-seni/" + sekretaris_pertandingan.id_penampilan_seni,
					function (data, textStatus, jqXHR) {
	...
	batalkan_diskualifikasi_peserta: function () {
		Swal.fire({
			...
			if (result.value) {
				$.post("sekretaris-pertandingan/batalkan-diskualifikasi-penampilan-seni/" + sekretaris_pertandingan.id_penampilan_seni,
					function (data, textStatus, jqXHR) {
						if (data.status == true) {
	...
	open_modal_set_manual_waktu: function () {
		if (sekretaris_pertandingan.status_penampilan == 'sedang_tampil') {
			...
			sekretaris_pertandingan.set_label_modal_atur_manual_waktu(sekretaris_pertandingan.waktu_tampil);
	...
	tetapkan_perubahan_manual_waktu: function () {
		...
		$.post("sekretaris-pertandingan/toggle-timer-seni/" + sekretaris_pertandingan.id_penampilan_seni,
			{ 'status_penampilan': 'berhenti', 'waktu_tampil': sekretaris_pertandingan.waktu_tampil},
			function (data, textStatus, jqXHR) {
				if (data.status == true) {
					setTimeout(() => {
						waitingDialog.hide();
	...
	refresh_status_seni: function () {
		...
		if ($waktu == null || $waktu == 0) {
			...
			*/
			$.post("sekretaris-pertandingan/refresh-status-seni/" + sekretaris_pertandingan.id_penampilan_seni,
				function (data, textStatus, jqXHR) {
