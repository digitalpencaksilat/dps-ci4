const sekretaris_pertandingan = {
  ...
  audio: {
    ...
    putar_gong: function () {
      ...
      if (
        sekretaris_pertandingan.waktu_sekarang ==
          sekretaris_pertandingan.pertandingan.waktu_per_ronde * 1000 ||
        sekretaris_pertandingan.waktu_sekarang == 0
      ) {
    ...
    putar_beep_alarm: () => {
      if (
        sekretaris_pertandingan.audio.beep_alarm != "0" &&
  ...
  websocket: {
    ...
    init: function ($WSserver, $id_gelanggang) {
      ...
      sekretaris_pertandingan.websocket.connection.onopen = function (event) {
        ...
        sekretaris_pertandingan.websocket.joinChannel(
          sekretaris_pertandingan.websocket.channel,
        );
      };

      sekretaris_pertandingan.websocket.connection.onmessage = function (
        event,
      ) {
  ...
  init: function ($pertandingan) {
    ...
    sekretaris_pertandingan.modal_ubah_waktu = new bootstrap.Modal(
      document.getElementById("modal_ubah_waktu"),
    );

    sekretaris_pertandingan.set_variable($pertandingan);
    
    if (sekretaris_pertandingan.socket) {
        sekretaris_pertandingan.socket.emit('JOIN_ROOM', sekretaris_pertandingan.id_pertandingan);
        
        sekretaris_pertandingan.socket.emit('KONTROL_WAKTU', {
            id_pertandingan: sekretaris_pertandingan.id_pertandingan,
            action: sekretaris_pertandingan.status_pertandingan,
            waktu: sekretaris_pertandingan.waktu_sekarang
        });
    }
  ...
  set_variable: function ($pertandingan) {
    ...
    sekretaris_pertandingan.status_pertandingan =
    ...
    sekretaris_pertandingan.waktu_sekarang =
      sekretaris_pertandingan.data_waktu[
        sekretaris_pertandingan.ronde_sekarang
      ][1];
  },
  set_timer: function () {
    $(".button-play-state").removeAttr("disabled");
    ...
    if (sekretaris_pertandingan.waktu_sekarang == 0) {
      ...
      let seconds = Math.floor(sekretaris_pertandingan.waktu_sekarang / 1000);
      ...
      if (seconds <= 0) {
          ...
          sekretaris_pertandingan.stopwatch.timer({
              ...
              callback: function() {
                  ...
                  $data_waktu[sekretaris_pertandingan.ronde_sekarang][1] = 0;
  ...
  mulai_pertandingan: function ($id_pertandingan) {
    Swal.fire({
      ...
      if (result.value) {
        $.post(
          "sekretaris-pertandingan/mulai-pertandingan/" + $id_pertandingan,
  ...
  toggle_timer: function () {
    let state = sekretaris_pertandingan.stopwatch.data("state");
    // Prevent starting if time is 0
    if (sekretaris_pertandingan.waktu_sekarang <= 0 && state !== "running") {
        return;
    }
    ...
    let currentSecs = sekretaris_pertandingan.stopwatch.data("seconds");
    if (currentSecs !== undefined) {
      sekretaris_pertandingan.waktu_sekarang = currentSecs * 1000;
    }
    ...
    if (sekretaris_pertandingan.socket) {
      ...
      console.log('Emit KONTROL_WAKTU:', sekretaris_pertandingan.status_pertandingan, jQuery.type(sekretaris_pertandingan.waktu_sekarang));
    }
  ...
  reset_timer: function () {
    Swal.fire({
      ...
      if (result.value) {
        ...
        $data_waktu = sekretaris_pertandingan.data_waktu;
        $data_waktu[sekretaris_pertandingan.ronde_sekarang][1] =
          sekretaris_pertandingan.pertandingan.waktu_per_ronde * 1000;
        ...
        if (sekretaris_pertandingan.socket) {
            sekretaris_pertandingan.socket.emit('KONTROL_WAKTU', {
                id_pertandingan: sekretaris_pertandingan.id_pertandingan,
                action: 'berhenti',
                waktu: sekretaris_pertandingan.pertandingan.waktu_per_ronde * 1000
            });
        }

        sekretaris_pertandingan.websocket.update_timer(
          sekretaris_pertandingan.pertandingan.waktu_per_ronde * 1000,
          "berhenti",
        ); // !!! Langsung perintah reset via websocket

        $.post(
          "sekretaris-pertandingan/toggle-timer-tanding/" +
            sekretaris_pertandingan.id_pertandingan,
          {
            status_pertandingan: "berhenti",
            data_waktu: JSON.stringify($data_waktu),
          },
    ...
    if (sekretaris_pertandingan.websocket.connection == null) {
      $.post(
        "sekretaris-pertandingan/toggle-timer-tanding/" +
          sekretaris_pertandingan.id_pertandingan,
        {
          status_pertandingan: sekretaris_pertandingan.status_pertandingan,
          data_waktu: JSON.stringify(sekretaris_pertandingan.data_waktu),
        },
        function (data, textStatus, jqXHR) {
          if (data.status == true) {
  ...
  open_modal_set_manual_waktu: function () {
    if (sekretaris_pertandingan.status_pertandingan == "berlangsung") {
      ...
      $('#formManualAturWaktu [name="detik"]').val(
        parseInt(sekretaris_pertandingan.waktu_sekarang) / 1000,
      );
  ...
  update_manual_waktu_sekarang: function ($el) {
    ...
    sekretaris_pertandingan.waktu_sekarang = $waktu_sekarang_terbaru * 1000;
    
    // Broadcast quick time change
    if (sekretaris_pertandingan.socket) {
        sekretaris_pertandingan.socket.emit('KONTROL_WAKTU', {
            id_pertandingan: sekretaris_pertandingan.id_pertandingan,
            action: sekretaris_pertandingan.status_pertandingan,
  ...
  pindah_ronde: function ($ronde_tujuan) {
    if (sekretaris_pertandingan.status_pertandingan == "berlangsung") {
      Swal.fire(
        "Error",
        "Stopwatch is still running, please stop it first !",
        "error",
      );
    } else {
      ...
      if ($ronde_tujuan <= $jumlah_ronde && $ronde_tujuan > 0) {
        Swal.fire({
          ...
          if (result.value) {
            $.post(
              "sekretaris-pertandingan/pindah-ronde-tanding/" +
                sekretaris_pertandingan.id_pertandingan,
              {
  ...
  update_tampilan_ronde: function () {
    ...
    $("#btn_round_" + sekretaris_pertandingan.ronde_sekarang)
      .addClass("bg-warning")
      .removeClass("bg-dark");
  },
  pindah_partai: function ($id_pertandingan) {
    ...
    $.post(
      "sekretaris-pertandingan/pindah-partai-tanding/" + $id_pertandingan,
      function (data, textStatus, jqXHR) {
        if (data.status == true) {
          window.location.reload();
        } else {
  ...
  refresh_status_pertandingan: () => {
    let secs = sekretaris_pertandingan.stopwatch.data("seconds");
    if (secs !== undefined) {
        $waktu_sekarang = secs * 1000;
    } else {
        $waktu_sekarang = sekretaris_pertandingan.waktu_sekarang;
    }
    ...
    $.post(
      "perangkat-pertandingan/refresh-status-pertandingan/" +
        sekretaris_pertandingan.id_pertandingan,
      {
        data_waktu:
          sekretaris_pertandingan.status_pertandingan == "berlangsung"
            ? JSON.stringify($data_waktu)
            : null,
      },
      function (data, textStatus, jqXHR) {
        if (data.status === true && data.reload === true) {
          ...
          data.status == true &&
          typeof data.pindah_partai != "undefined" &&
          data.pindah_partai == true
        ) {
