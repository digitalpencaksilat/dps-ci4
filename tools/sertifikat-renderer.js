#!/usr/bin/env node
/**
 * Renderer lokal sertifikat — screenshot tiap .sertifikat-export via Playwright,
 * kemas dalam ZIP per chunk.
 *
 * Usage:
 *   node tools/sertifikat-renderer.js --input file.html --output out-dir --scale 2 --chunk-size 20 --progress-file progress.json
 */
const fs = require('fs');
...
async function screenshotElementByClip(page, element, timeoutMs) {
  const handle = await element.evaluateHandle((source) => {
    const cloneId = '__sertifikat_render_clone__';
    const old = document.getElementById(cloneId);
    if (old) old.remove();

    const clone = source.cloneNode(true);
    clone.id = cloneId;
    clone.classList.add('__sertifikat-render-clone');
  ...
  if (!clone) {
    throw new Error('Clone sertifikat tidak bisa dibuat.');
  }
  ...
  if (!box || box.width <= 0 || box.height <= 0) {
    ...
    throw new Error('Bounding box sertifikat tidak ditemukan.');
  }

  const previousViewport = page.viewportSize() || { width: 2480, height: 1754 };
  ...
  if (!freshBox || freshBox.width <= 0 || freshBox.height <= 0) {
    await clone.evaluate((el) => el.remove()).catch(() => {});
    throw new Error('Bounding box sertifikat tidak ditemukan setelah clone.');
  }
...
async function main() {
  ...
  const scale = Math.max(1, parseInt(arg('scale', '2'), 10) || 2);
  const chunkSize = Math.max(1, parseInt(arg('chunk-size', '20'), 10) || 20);
  const loadTimeout = Math.max(5000, parseInt(arg('load-timeout', '120000'), 10) || 120000);
  const assetTimeout = Math.max(1000, parseInt(arg('asset-timeout', '20000'), 10) || 20000);

  if (!input || !output) {
    throw new Error('Usage: node tools/sertifikat-renderer.js --input file.html --output output-dir --scale 2 --chunk-size 20 --progress-file progress.json');
  }
  ...
  const page = await browser.newPage({
    viewport: { width: 2480, height: 1754 }, // A4 landscape @ ~300dpi
    deviceScaleFactor: scale,
  });
  page.setDefaultTimeout(loadTimeout);
  page.setDefaultNavigationTimeout(loadTimeout);

  reportProgress(progressFile, { status: 'loading', stage: 'Memuat HTML Sertifikat', total: 0, processed: 0, zip_files: [] });
  await page.goto(`file://${path.resolve(input)}`, { waitUntil: 'domcontentloaded', timeout: loadTimeout });
  await page.waitForSelector('.sertifikat-export', { timeout: loadTimeout });

  reportProgress(progressFile, { status: 'loading', stage: 'Memuat font, gambar, QR code', total: 0, processed: 0, zip_files: [] });
  await waitForAssets(page, assetTimeout);
  await neutralizeFontWait(page);
  // Tunggu QR code JS render
  await page.waitForTimeout(500);

  const cards = await page.$$('.sertifikat-export');
  let part = 1;
  let zip = new JSZip();
  let countInPart = 0;
  let processed = 0;
  ...
  if (cards.length === 0) {
    throw new Error('Tidak ada elemen .sertifikat-export di HTML input.');
  }

  reportProgress(progressFile, { status: 'rendering', stage: 'Mulai render sertifikat', total: cards.length, processed, total_parts: totalParts, current_part: part, zip_files: zipFiles });

  for (let i = 0; i < cards.length; i++) {
    const card = cards[i];
    const filename = await card.evaluate((el, idx) => el.dataset.filename || `sertifikat_${idx + 1}`, i);
    ...
    if (countInPart >= chunkSize || i === cards.length - 1) {
      ...
      const zipName = totalParts === 1
        ? 'sertifikat-batch.zip'
        : `sertifikat-batch-part-${paddedPart}-of-${paddedTotal}.zip`;
